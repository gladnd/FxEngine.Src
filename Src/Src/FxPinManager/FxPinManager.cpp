...
#define FXMANAGER_VERSION        "4.0.0.0"	//<! Y: compnent interface, Z: component code
/* History */
/*	1.1.0.0 (28/05/2006)
...
*/
/*	4.0.0.0 (28/09/2010)
	Split the RemovePin method in RemoveInPin and RemoveOutPin methods.
*/
/*	4.1.0.0 (20/08/2011)
	Fixed the case where there are two pins of the same Fx which are connected
*/
...
Int32 CFxPinManager::Create(PFX_PIN pPinInfo, IFxPin** ppFxPin)
{
	...
	*ppFxPin = NULL;
	...
	if(pPinInfo)
	{
		...
		Int32 hr;
		if(pPinInfo->PinType == PIN_IN)
			pFxPin = new CFxInPin(pPinInfo, _strFxName, _dwFxEngineInstance, _dwFxInstance, _dwPinInstance++, hr);
		else
			pFxPin = new CFxOutPin(pPinInfo, _strFxName, _dwFxEngineInstance, _dwFxInstance, _dwPinInstance++, hr);
		if(FEF_FAILED(hr))
		{
...
Int32 CFxPinManager::Remove(IFxPin* pFxPin)
{
	...
	if(PinType == PIN_IN) {
		if( FEF_FAILED(hr = RemoveInPin(pFxPin)))
			return hr;
	}
	else {
		if( FEF_FAILED(hr = RemoveOutPin(pFxPin)))
			return hr;
	}

	_pLogTrace->FxTrace( "FxPinManager::Remove quit");

	return FX_OK;
}

Int32 CFxPinManager::RemoveInPin(IFxPin* pFxPin)
{
	Int32 hr;
	...
	if( Iter != _lpFxPin.end() )
	{
        ...
        if(pFxPinConnectedClass) {
			...
			if(dwConnectionNumber == 1) //!< It's time to stop the output pin
				pFxPinConnectedClass->SetPinRunState(FX_STOP);
        }

        /*! Quit infinite waiting time for all pins of both Fx */
		CFxPinIter IterPin;
		...
		if(pFxPinConnectedClass && (dwConnectionNumber == 1)) {
			FX_HANDLE hFx;
		    FX_COMPONENT* pFx;
			hFx = pFxPinConnectedClass->GetFxMember();
			pFx = static_cast<FX_COMPONENT*>(hFx);
			Uint16 wPinCount;
			if( FEF_FAILED(hr = pFx->pFxPinManager->GetPinCount(wPinCount, (pFx != _hFx))) ) {
			...
			for(Int16 Idx = 0; Idx < wPinCount; Idx++)
			{
				...
				pFx->pFxPinManager->GetPin(&pFxPin, Idx, (pFx != _hFx));
				if(pFxPin) {
        ...
        if(pFxPinConnectedClass) {
			pFxPinConnectedClass->DisconnectPin(pFxPin); //!< Remove the input connection
...
Int32 CFxPinManager::RemoveOutPin(IFxPin* pFxPin)
{
	Int32 hr;
	...
	if( Iter != _lpFxPin.end() )
	{
        ...
        /*! Stop, disconnect and detach observer pin */
        pFxPinClass->SetPinRunState(FX_STOP);
        pFxPinClass->_ShoudStopCallBack = TRUE;
        while(pFxPinConnectedClass) {
			pFxPinConnectedClass->SetPinRunState(FX_STOP);
			pFxPinConnected = NULL;
			pFxPinClass->GetNextPinConnected(&pFxPinConnected);
			pFxPinConnectedClass = NULL;
			pFxPinConnectedClass = static_cast<CFxPin*>(pFxPinConnected);
		}
		...
		/*! (2) Call DisconnectPin */
        pFxPinConnected = NULL;
	    pFxPinClass->GetPinConnected(&pFxPinConnected);
		pFxPinConnectedClass = NULL;
		pFxPinConnectedClass = static_cast<CFxPin*>(pFxPinConnected);
        while(pFxPinConnectedClass) {
			pFxPinClass->DisconnectPin(pFxPinConnectedClass);
			pFxPinClass->DetachObservers(NULL);
		    pFxPinConnectedClass->DisconnectPin();
			pFxPinConnected = NULL;
			pFxPinClass->GetNextPinConnected(&pFxPinConnected); //!< pFxPinConnected is an input pin
			pFxPinConnectedClass = NULL;
			pFxPinConnectedClass = static_cast<CFxPin*>(pFxPinConnected);
	    }
...
Int32 CFxPinManager::GetPinCount(Uint16& wPinCount, Bool ShouldProtect/* = TRUE*/)
{
...
Int32 CFxPinManager::GetPin(IFxPin** ppIFxPin, Uint16 wPinNumber, Bool ShouldProtect/* = TRUE*/)
{
	AutoLock lock(_CS, ShouldProtect);
...
Void CFxPinManager::ClearFxPinList()
{
	...
	/*! ClearFxPinList is called on ~CFx.  */
	CFxPinIter Iter;
    CFxPin* pFxPinClass = NULL;

	/*! Case where there are two pins of the same Fx which are connected */
    for (Iter = _lpFxPin.begin(); (Iter != _lpFxPin.end()) && !(_lpFxPin.empty()); Iter++){
        pFxPinClass = (*Iter);
		pFxPinClass->DetachObservers(NULL); //!< If not, the engine core will ask for CFx interface
		pFxPinClass->_ShoudStopCallBack = TRUE; //!< If not, 
	}
