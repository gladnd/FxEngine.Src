...
#define FX_VERSION        "3.0.0.0"  //<! W: Fx/CFx interface, X: Fx/CFx code, Y: compnent interface, Z: component code
/* History */
/*	2.0.0.0 (01/05/2006)
...
/*  3.0.0.0 (07/08/2012)
    Added FxNote
	Added APC object in the Fx to have only one message queue for state and note
*/
namespace
{
...
Int32 CFx::FxGetInterface(FX_INTERFACE FxInterfaceType, Void** ppFxInterface)
{
	...
	*ppFxInterface = NULL;
	...
	else if(FxInterfaceType == IFX_NOTE)
		*ppFxInterface = static_cast<IFxNote*>( _pFxObject );
	else
	{
	...
	case IFX_STATE:
		...
		break;
	case IFX_NOTE:
        _pLogTrace->FxTrace( "Fx::GetInterface IFX_NOTE");
		break;
	}
...
CFx::CFx(const std::string strFxName, CFxAPC* pFxAPC):
...
CFxState(strFxName, CFxEngineCore::_dwInstanceCount, _dwInstanceCount, pFxAPC),
CFxNote(strFxName, CFxEngineCore::_dwInstanceCount, _dwInstanceCount, pFxAPC),
_pFxAPC(pFxAPC)
{
	_pFxObject = NULL;
 
	_dwThisInstance = CFx::_dwInstanceCount; //!< For message trace only
...
FXENGINE_EXP Int32 FXENGINE_API FEF_CreateFx(IFx ** ppFx, const std::string strFxName)
{
	...
	try {
		//*! Create CFxAPC object */
		CFxAPC* pFxAPC = NULL;
		pFxAPC = new CFxAPC();
		
		CFx* pFx = new CFx(strFxName, pFxAPC);
		if(pFx)
		{
