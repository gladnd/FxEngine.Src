...
#define TOGGLE_LOCK(A, B) (A).lock(); (B).unlock();
#define TOGGLE_LOCK_COND(A, B, C) (A).lock(); if(C) (B).unlock();
#define TOGGLE_UNLOCK(A, B) (A).unlock(); (B).lock();
#define TOGGLE_UNLOCK_COND(A, B, C) (A).unlock(); if(C) (B).lock();
...
CFxEngineCore::~CFxEngineCore(void)
{
	...
	/*! Clear State and Note observers */
	FxObserverSet::iterator It = _FxObserverSet.begin();
	while(It != _FxObserverSet.end())
	{
		SAFE_DELETE(*It);
		It++;
	}
	_FxObserverSet.clear();
	...
	*phFx = NULL;
	...
	if (pFxComponent->hDLL)
	{
		...
		else
		{
			...
			TOGGLE_LOCK_COND(*pFxComponent, lock, ShouldProtect);
			hr = pFxComponent->pFxBase->InitFx(&pFxComponent->pIFx);
			TOGGLE_UNLOCK_COND(*pFxComponent, lock, ShouldProtect);
			if(FEF_FAILED(hr))
			{
			...
			/*! Attach State and Note mechanism */
			hr = AttachFxObserver(pFxComponent);
			if(FEF_FAILED(hr))
			{
				strError = "\tFxEngine::FEF_AddFx FEF_FAILED !! (AttachFxObserver Failed)";
				goto Error;
			}
...
*/
			{
				*phFx = (FEF::FX_HANDLE*)pFxComponent;
...
Int32 CFxEngineCore::AddFxEx(IFxBase* pIFxBase, FX_HANDLE* phFx)
{
	...
	*phFx = NULL;
	...
	TOGGLE_LOCK(*pFxComponent, lock);
	hr = pFxComponent->pFxBase->InitFx(&pFxComponent->pIFx);
	TOGGLE_UNLOCK(*pFxComponent, lock);
	if(FEF_FAILED(hr))
	{
...
Int32 CFxEngineCore::RemoveFx(FX_HANDLE hFx)
{
	...
	if(IsFxLoaded(hFx))
	{
		...
		TOGGLE_LOCK(*pFx, lock);
		hr = pFx->pFxBase->ReleaseFx();
		TOGGLE_UNLOCK(*pFx, lock);
		if ( FEF_FAILED(hr) )
		{
...
Int32 CFxEngineCore::StartFx(FX_HANDLE hFx, Bool ShouldProtect/* = TRUE*/)
{
	...
	if(IsFxLoaded(hFx))
	{
		...
		for(Int16 Idx = 0; Idx < wPinCount; Idx++)
		{
			...
			{
				pFxPinClass = static_cast<CFxPin*>(pFxPin);
				pFxPinClass->SetPinRunState(FX_START);
			}
		}

		TOGGLE_LOCK_COND(*pFx, lock, ShouldProtect);
		hr = pFx->pFxBase->StartFx();
		TOGGLE_UNLOCK_COND(*pFx, lock, ShouldProtect);
        if( FEF_FAILED(hr) )
		{
		    ...
		    for(Int16 Idx = 0; Idx < wPinCount; Idx++) {
			    ...
			    if(pFxPin) {
				    pFxPinClass = static_cast<CFxPin*>(pFxPin);
				    pFxPinClass->SetPinRunState(FX_STOP);
			    }
		    }
			return FX_ERROR;
		}
...
Int32 CFxEngineCore::StartFxEngine()
{
	...
	for (Iter = _FxVector.begin(); (Iter != _FxVector.end()) && !(_FxVector.empty()); Iter++)
	{
		TOGGLE_LOCK(**Iter, lock);
		hr = (*Iter)->pFxBase->StartFx();
		TOGGLE_UNLOCK(**Iter, lock);
		if( FEF_FAILED(hr) )
		{
			_pLogTrace->FxTrace(  "\tCFxEngineCore::StartFxEngine FEF_FAILED !!, hr=%d (Cannot Start Fx)", hr);
			SetFxState((FX_PTR)(*Iter), FX_ERROR_STATE);
			//return FX_ERROR; //!< 2011 04: start all Fx even if one is failed (4.8)
            /*! And stop pin state */
            Uint16 wPinCount;
		    if( FEF_FAILED(hr = (*Iter)->pFxPinManager->GetPinCount(wPinCount)) ) {
			    _pLogTrace->FxTrace(  "\tCFxEngineCore::StartFxEngine FEF_FAILED !! (Cannot get pin count)");
            ...
            startedFx.push_back((*Iter));
	}

    /*! Run all started Fx */
	for (Iter = startedFx.begin(); (Iter != startedFx.end()) && !(startedFx.empty()); Iter++)
	{
		TOGGLE_LOCK(**Iter, lock);
		hr = (*Iter)->pFxBase->RunFx();
		TOGGLE_UNLOCK(**Iter, lock);
		if( FEF_FAILED(hr) )
		{
...
Int32 CFxEngineCore::PauseFx(FX_HANDLE hFx, Bool ShouldProtect/* = TRUE*/)
{
	...
	if(IsFxLoaded(hFx))
	{
		...
		for(Int16 Idx = 0; Idx < wPinCount; Idx++)
		{
			...
			{
				pFxPinClass = static_cast<CFxPin*>(pFxPin);
				pFxPinClass->SetPinRunState(FX_PAUSE);
			}
		}
...
Int32 CFxEngineCore::StopFxEngine()
{
	...
	for (Iter = _FxVector.begin(); (Iter != _FxVector.end()) && !(_FxVector.empty()); Iter++)
	{
		_pLogTrace->FxTrace(  "Stop Fx entry - %s", (*Iter)->strFxName.c_str());
		Uint16 wPinCount;
		if( FEF_FAILED(hr = (*Iter)->pFxPinManager->GetPinCount(wPinCount)) )
		{
			_pLogTrace->FxTrace(  "\tCFxEngineCore::StopFxEngine FEF_FAILED !! (Cannot get pin count)");
        ...
        /*! Wait for the end of pin data */
		_pLogTrace->FxTrace(  "\tWaiting for pin stopping entry");
        for(Int16 Idx = 0; Idx < wPinCount; Idx++)
		{
			IFxPin* pFxPin = NULL;
			CFxPin* pFxPinClass = NULL;
		...
		_pLogTrace->FxTrace(  "\tWaiting for pin stopping quit");

		_pLogTrace->FxTrace(  "\tIFxBase::StopFx entry");
		TOGGLE_LOCK(**Iter, lock);
		hr = (*Iter)->pFxBase->StopFx();
		TOGGLE_UNLOCK(**Iter, lock);
		if( FEF_FAILED(hr) )
		{
		...
		_pLogTrace->FxTrace(  "\tIFxBase::StopFx quit");

		/*! Save Fx Pins state */
		(*Iter)->FxState   = FX_STOP;
	}
...
Int32 CFxEngineCore::GetFxInfo(FX_HANDLE hFx, const FX_DESCRIPTOR** ppFxDescriptor)
{
	...
	if(IsFxLoaded(hFx))
	{
		...
		TOGGLE_LOCK(*pFx, lock);
		Int32 hr = pFx->pFxBase->GetFxInfo(ppFxDescriptor);
		TOGGLE_UNLOCK(*pFx, lock);
        return (hr);
	}
	else
...
Void CFxEngineCore::SetFxState(FX_PTR dwFx, FX_STATE FxState)
{
	...
	/*! Get the Fx state */
	IFxState* pFxState;
    Int32 hr = ((FX_COMPONENT*)dwFx)->pIFx->FxGetInterface(IFX_STATE, (Void**)&pFxState);
	if(FEF_FAILED(hr)) {
        _pLogTrace->FxTrace("\tFxEngine::SetFxState FEF_FAILED !! (IFx::FxGetInterface IFX_State FEF_FAILED !!)");
        return;
    }
    CFxState* pClassFxState = static_cast<CFxState*>(pFxState);
	pFxState->FxPublishState(FxState);
	
	_pLogTrace->FxTrace(  "CFxEngineCore::SetFxState quit");
	return;
}

Int32 CFxEngineCore::GetFxRunningState(FX_HANDLE hFx, FX_RUNNING_STATE* pFxRunningState, Bool ShouldProtect/* = TRUE*/)
{
	AutoLock lock(_CSObserver, ShouldProtect);
    
    _pLogTrace->FxTrace(  "CFxEngineCore::GetFxRunningState entry");
	if(pFxRunningState == NULL)
	{
		_pLogTrace->FxTrace(  "\tFxEngine::GetFxRunningState FEF_FAILED !! (Invalid Parameter: pFxRunningState)");
		return FX_INVALPARAM;
	}

    if(IsFxLoaded(hFx)) {
		FX_COMPONENT* pFx = static_cast<FX_COMPONENT*>(hFx);
        *pFxRunningState = pFx->FxState;        
    }
    else {
        _pLogTrace->FxTrace(  "\tFxEngine::GetFxRunningState FEF_FAILED !! (Invalid Parameter: hFx)");
		return FX_INVALPARAM;
    }

	_pLogTrace->FxTrace(  "CFxEngineCore::GetFxRunningState quit");
...
Int32 CFxEngineCore::GetFxState(FX_HANDLE hFx, FX_STATE* pFxState, Bool ShouldProtect/* = TRUE*/)
{
	//AutoLock lock(_CSObserver, ShouldProtect);
    AutoLock lock(_CS);
	//_pLogTrace->FxTrace(  "CFxEngineCore::GetFxState entry"); //!< too much log
	if(pFxState == NULL)
	{
	...
	if( Iter != _FxVector.end() )
	{
        ...
        Int32 hr = (*Iter)->pIFx->FxGetInterface(IFX_STATE, (Void**)&pIFxState);
		if(FEF_FAILED(hr)) {
            _pLogTrace->FxTrace("\tFxEngine::SetFxState FEF_FAILED !! (IFx::FxGetInterface IFX_State FEF_FAILED !!)");
            return FX_ERROR;
        }
        CFxState* pClassFxState = static_cast<CFxState*>(pIFxState);
		pClassFxState->FxGetState(pFxState);
		return FX_OK;
	}
...
Int32 CFxEngineCore::AttachFxObserver(CFxStateCallback* pFxStateCallback, CFxNoteCallback* pFxNoteCallback, FX_PTR dwParam, FX_HANDLE* phObserverId)
{
	AutoLock lock(_CSObserver);
	...
	/*!For all Fx */
	AutoLock lockCS(_CS);
	/*! Allocate Observer ID */
	FX_OBSERVER* pObserverId = new FX_OBSERVER;
	for (ItFx = _FxVector.begin(); (ItFx != _FxVector.end()) && !(_FxVector.empty()); ItFx++)
	{
		_pLogTrace->FxTrace("\tFxEngine::AttachObserver %s", (*ItFx)->strFxName.c_str());
		if(FEF_FAILED( hr = AttachFxObserver(*ItFx, pFxStateCallback, pFxNoteCallback, dwParam, pObserverId)))
			return FX_ERROR;
	}

	pObserverId->pFxNoteClass = pFxNoteCallback;
	pObserverId->pFxStateClass = pFxStateCallback;
	pObserverId->dwParam = dwParam ;
	pObserverId->pFxNoteFnct = NULL;
	pObserverId->pFxStateFnct = NULL;
	_FxObserverSet.insert(pObserverId);

	*phObserverId = (FX_HANDLE)pObserverId;

	lockCS.unlock();
	

	_pLogTrace->FxTrace(  "CFxEngineCore::AttachObserver quit");
	return FX_OK;	
}

Int32 CFxEngineCore::AttachFxObserver(FX_COMPONENT* pFx, CFxStateCallback* pFxStateCallback, CFxNoteCallback* pFxNoteCallback, FX_PTR dwParam, FX_OBSERVER* phObserverId)
{
	Int32 hr = FX_OK;
	...
	if(pFxStateCallback) {
		IFxState* pFxState;
		hr = pFx->pIFx->FxGetInterface(IFX_STATE, (Void**)&pFxState);
		if(FEF_FAILED(hr)) {
			_pLogTrace->FxTrace("\tFxEngine::Attach Observer FEF_FAILED !! (IFx::FxGetInterface IFX_State FEF_FAILED !!)");
			return FX_ERROR;
		}
		CFxState* pClassFxState = static_cast<CFxState*>(pFxState);

		if(FEF_FAILED( hr = pClassFxState->AttachFxObserver(pFxStateCallback, dwParam, phObserverId)) )
		{
			_pLogTrace->FxTrace(  "\tFxEngine::Attach Observer FEF_FAILED !!, hr=%d", hr);
			return FX_ERROR;
		}
	}
	...
	if(pFxNoteCallback) {
		IFxNote* pFxNote;
		hr = pFx->pIFx->FxGetInterface(IFX_NOTE, (Void**)&pFxNote);
		if(FEF_FAILED(hr)) {
			_pLogTrace->FxTrace("\tFxEngine::Attach Observer FEF_FAILED !! (IFx::FxGetInterface IFX_Note FEF_FAILED !!)");
			return FX_ERROR;
		}
		CFxNote* pClassFxNote = static_cast<CFxNote*>(pFxNote);

		if(FEF_FAILED( hr = pClassFxNote->AttachFxObserver(pFxNoteCallback, dwParam, phObserverId)) )
		{
			_pLogTrace->FxTrace(  "\tFxEngine::Attach Observer FEF_FAILED !!, hr=%d", hr);
			return FX_ERROR;
		}
...
Int32 CFxEngineCore::AttachFxObserver(FX_COMPONENT* pFx)
{
	AutoLock lock(_CSObserver);
	_pLogTrace->FxTrace(  "CFxEngineCore::AttachFxObserver entry");

	IFxState* pFxState;
	Int32 hr = pFx->pIFx->FxGetInterface(IFX_STATE, (Void**)&pFxState);
	if(FEF_FAILED(hr)) {
		_pLogTrace->FxTrace("\tFxEngine::Attach Observer FEF_FAILED !! (IFx::FxGetInterface IFX_State FEF_FAILED !!)");
		return FX_ERROR;
	}
	CFxState* pClassFxState = static_cast<CFxState*>(pFxState);
	pClassFxState->SetFxHandle(pFx);
	...
	while(It != _FxObserverSet.end())
	{
		/*! IFXSTATE */
		if((*It)->pFxStateClass) {
			if(FEF_FAILED( hr = pClassFxState->AttachFxObserver((*It)->pFxStateClass, (*It)->dwParam, *It)) )
			{
				_pLogTrace->FxTrace(  "\tFxEngine::Attach Observer FEF_FAILED !!, hr=%d", hr);
				return FX_ERROR;
			}
		...
		if((*It)->pFxStateFnct) {
			if(FEF_FAILED( hr = pClassFxState->AttachFxObserverEx((*It)->pFxStateFnct, (*It)->dwParam, *It)) )
			{
				_pLogTrace->FxTrace(  "\tFxEngine::Attach Observer Ex FEF_FAILED !!, hr=%d", hr);
				return FX_ERROR;
			}
		}
		...
		/*! IFXNOTE */
		if((*It)->pFxNoteClass) {
			if(FEF_FAILED( hr = pClassFxNote->AttachFxObserver((*It)->pFxNoteClass, (*It)->dwParam, *It)) )
			{
				_pLogTrace->FxTrace(  "\tFxEngine::Attach Observer FEF_FAILED !!, hr=%d", hr);
				return FX_ERROR;
			}
		...
		if((*It)->pFxNoteFnct) {
			if(FEF_FAILED( hr = pClassFxNote->AttachFxObserverEx((*It)->pFxNoteFnct, (*It)->dwParam, *It)) )
			{
				_pLogTrace->FxTrace(  "\tFxEngine::Attach Observer Ex FEF_FAILED !!, hr=%d", hr);
				return FX_ERROR;
			}
...
Int32 CFxEngineCore::AttachFxObserverEx(FXSTATECALLBACK* pFxStateCallback, FXNOTECALLBACK* pFxNoteCallback, FX_PTR dwParam, FX_HANDLE* phObserverId)
{
	AutoLock lock(_CSObserver);
	_pLogTrace->FxTrace(  "CFxEngineCore::AttachObserverEx entry");

	Int32 hr = FX_OK;

	if( (pFxStateCallback == NULL) && (pFxNoteCallback == NULL) )
	{
		_pLogTrace->FxTrace(  "\tFxEngine::AttachObserverEx FEF_FAILED !! (Invalid Parameters: pFxStateCallback, pFxNoteCallback)");
		return FX_INVALPARAM;
	}
	
	if(phObserverId == NULL)
	{
		_pLogTrace->FxTrace(  "\tFxEngine::AttachObserverEx FEF_FAILED !! (Invalid Parameter: phObserverId)");
		return FX_INVALPARAM;
	}
	...
	for (ItFx = _FxVector.begin(); (ItFx != _FxVector.end()) && !(_FxVector.empty()); ItFx++)
	{
		_pLogTrace->FxTrace("\tFxEngine::AttachObserverEx %s", (*ItFx)->strFxName.c_str());

		/*! Allocate Observer ID */
		if(FEF_FAILED( hr = AttachFxObserverEx(*ItFx, pFxStateCallback, pFxNoteCallback, dwParam, pObserverId)))
			return FX_ERROR;
	}
	pObserverId->pFxNoteFnct = pFxNoteCallback;
	pObserverId->pFxStateFnct = pFxStateCallback;
	pObserverId->dwParam = dwParam ;
	pObserverId->pFxNoteClass = NULL;
	pObserverId->pFxStateClass = NULL;
	_FxObserverSet.insert(pObserverId);

	*phObserverId = (FX_HANDLE)pObserverId;
	lockCS.unlock();
	
	_pLogTrace->FxTrace(  "CFxEngineCore::AttachObserverEx quit");
	return FX_OK;
}

Int32 CFxEngineCore::AttachFxObserverEx(FX_COMPONENT* pFx, FXSTATECALLBACK* pFxStateCallback, FXNOTECALLBACK* pFxNoteCallback, FX_PTR dwParam, FX_OBSERVER* pObserverId)
{
	Int32 hr = FX_OK;
	...
	if(pFxStateCallback) {
		...
		if(FEF_FAILED( hr = pClassFxState->AttachFxObserverEx(pFxStateCallback, dwParam, pObserverId)) )
		{
			_pLogTrace->FxTrace(  "\tFxEngine::Attach Observer FEF_FAILED !!, hr=%d", hr);
			return FX_ERROR;
		}
	...
	if(pFxNoteCallback) {
		...
		if(FEF_FAILED( hr = pClassFxNote->AttachFxObserverEx(pFxNoteCallback, dwParam, pObserverId)) )
		{
			_pLogTrace->FxTrace(  "\tFxEngine::Attach Observer FEF_FAILED !!, hr=%d", hr);
			return FX_ERROR;
		}
...
{
	AutoLock lock(_CSObserver);
	_pLogTrace->FxTrace(  "CFxEngineCore::DetachObserver entry");

	Int32 hr = FX_OK;

	FxObserverSet::iterator It;

	FX_OBSERVER* pObserver = (FX_OBSERVER*)hObserverId;
	It = _FxObserverSet.find( pObserver );
    if( It != _FxObserverSet.end() )
    {
		CFxVectorIter ItFx;
		/*!For all Fx */
		AutoLock lock(_CS);
		for (ItFx = _FxVector.begin(); (ItFx != _FxVector.end()) && !(_FxVector.empty()); ItFx++)
		{
			/*! IFXSTATE */
			IFxState* pFxState;
			hr = (*ItFx)->pIFx->FxGetInterface(IFX_STATE, (Void**)&pFxState);
			if(FEF_FAILED(hr)) {
				_pLogTrace->FxTrace("\tFxEngine::Detach Observer FEF_FAILED !! (IFx::FxGetInterface IFX_State FEF_FAILED !!)");
				return FX_ERROR;
			}
			CFxState* pClassFxState = static_cast<CFxState*>(pFxState);

			if(FEF_FAILED( hr = pClassFxState->DetachFxObserver(pObserver)) )
			{
				_pLogTrace->FxTrace(  "\tFxEngine::Detach Observer FEF_FAILED !!, hr=%d", hr);
				return FX_ERROR;
			}
		
			/*! IFXNOTE */
			IFxNote* pFxNote;
			hr = (*ItFx)->pIFx->FxGetInterface(IFX_NOTE, (Void**)&pFxNote);
			if(FEF_FAILED(hr)) {
				_pLogTrace->FxTrace("\tFxEngine::Detach Observer FEF_FAILED !! (IFx::FxGetInterface IFX_Note FEF_FAILED !!)");
				return FX_ERROR;
			}
			CFxNote* pClassFxNote = static_cast<CFxNote*>(pFxNote);

			if(FEF_FAILED( hr = pClassFxNote->DetachFxObserver(pObserver)) )
			{
				_pLogTrace->FxTrace(  "\tFxEngine::Detach Observer FEF_FAILED !!, hr=%d", hr);
				return FX_ERROR;
			}
		...
		lock.unlock();
		_FxObserverSet.erase(pObserver);
	}
	else
	{
		_pLogTrace->FxTrace(  "\tFxEngine::Detach Observer FEF_FAILED !! (Invalid Parameter: hObserverId)");
		return FX_ERROR;
	}
	
	_pLogTrace->FxTrace(  "CFxEngineCore::DetachObserver quit");
	return FX_OK;
}
...
Int32 CFxEngineCore::UpdateFxParam(FX_HANDLE hFx, const std::string strParamName, FX_PARAMETER FxParameter, Bool ShouldProtect/* = TRUE*/)
{
    ...
    if(IsFxLoaded(hFx))
	{
		...
		TOGGLE_LOCK_COND(*pFx, lock, ShouldProtect);
		hr = pFx->pFxBase->UpdateFxParam(strParamName, FxParameter);
		TOGGLE_UNLOCK_COND(*pFx, lock, ShouldProtect);
		if( FEF_FAILED(hr) )
		{
			SetFxState((FX_PTR)pFx, FX_ERROR_STATE);
...
Int32 CFxEngineCore::ConnectFxPin(IFxPin* pFxPinIn, IFxPin* pFxPinOut, Bool ShouldProtect/* = TRUE*/)
{
	...
	FX_PIN_STATE pFxInPinState;
	pFxPinInClass->GetPinState(&pFxInPinState);
	if(pFxInPinState == PIN_CONNECTED)
	{
		_pLogTrace->FxTrace(  "\tFxEngine::ConnectFxPin FEF_FAILED !! (Pin In is already connected)");
		return FX_INVALPARAM;
	}

	FX_PIN_STATE pFxOutPinState;
	pFxPinOutClass->GetPinState(&pFxOutPinState);

	Bool MediaTypeMatch = FALSE;

	if(pFxOutPinState == PIN_NOT_CONNECTED) {
		...
		for(Int16 IdxMediaOut = 0; IdxMediaOut < wMediaTypeCountPinOut; IdxMediaOut++)
		{
			...
			if( pFxPinInClass->CheckMediaType(&MediaTypeOut) == FX_OK)
			{
				...
				pFxPinOutClass->SetPinRunState(pFxOut->FxState);
				...
				/*! Set Pin state according to the Fx state */
				pFxPinInClass->SetPinRunState(pFxIn->FxState);
	...
	else { //!< Output pin is already connected, try to connect the new input pin only according to the media type
		/*! Get out media type */
		FX_MEDIA_TYPE MediaTypeOut;
		pFxPinOutClass->GetConnectionMediaType(&MediaTypeOut);
		if( pFxPinInClass->CheckMediaType(&MediaTypeOut) == FX_OK)
		{
			MediaTypeMatch = TRUE;		
            if( FEF_FAILED(hr = pFxPinInClass->SetConnectionMediaType(&MediaTypeOut)) )
...
Int32 CFxEngineCore::ConnectFxPinEx(IFxPin* pFxPinIn, IFxPin* pFxPinOut, PFX_MEDIA_TYPE pMediaType, Bool ShouldProtect/* = TRUE*/)
{
	...
	if(pFxOutPinState == PIN_NOT_CONNECTED)
	{
		...
		FX_MEDIA_TYPE MediaTypeOut;
		pFxPinOutClass->GetConnectionMediaType(&MediaTypeOut);
		if( (MediaTypeOut.MainMediaType == pMediaType->MainMediaType) &&
			(MediaTypeOut.SubMediaType == pMediaType->SubMediaType) )
		{
			MediaTypeMatch = TRUE;
			if( FEF_FAILED(hr = pFxPinInClass->SetConnectionMediaType(pMediaType)) )
			{
...
Int32 CFxEngineCore::DisConnectFxPin(IFxPin* pFxPin, Bool ShouldStopCallBack /*= TRUE*/, Bool ShouldProtect/* = TRUE*/)
{
	...
	FX_PIN_TYPE PinType;
	pFxPin->GetPinType(&PinType);
	if(PinType == PIN_IN) {
		if( FEF_FAILED(hr = DisConnectFxInPin(pFxPin, ShouldStopCallBack, ShouldProtect)))
			return hr;
	}
	else //!< DisConnectFxOutPin returns always FX_OK
		DisConnectFxOutPin(pFxPin, ShouldStopCallBack, ShouldProtect);
	
	_pLogTrace->FxTrace(  "CFxEngineCore::DisconnectFxPin quit");
	return FX_OK;
}

Int32 CFxEngineCore::DisConnectFxInPin(IFxPin* pFxPin, Bool ShouldStopCallBack /*= TRUE*/, Bool ShouldProtect/* = TRUE*/)
{
	Int32 hr = FX_OK;
	...
	Uint32 dwConnectionNumber = 0;
    if(pFxPinConnected) {
        pFxPinConnectedClass->GetConnectedPinNumber(&dwConnectionNumber);
		pFxPinConnectedClass = static_cast<CFxPin*>(pFxPinConnected);
		if(dwConnectionNumber == 1)
			pFxPinConnectedClass->SetPinRunState(FX_STOP);
    }
    ...
    if(ShouldStopCallBack == TRUE)
    {
		...
		if(pFxPinConnected && (dwConnectionNumber == 1)) {
    ...
    if(pFxPinConnected)
    {
        ...
        pFxPinConnectedClass = static_cast<CFxPin*>(pFxPinConnected);
        /*! Disconnect current pin first */
        if( FEF_FAILED(hr = pFxPinConnectedClass->DisconnectPin(pFxPin)) )
        {
	        _pLogTrace->FxTrace(  "\tFxEngine::DisconnectFxPin FEF_FAILED !! (IFxPin Cannot disconnect current Pin !!)");
	        return FX_ERROR;
        }
...
Int32 CFxEngineCore::DisConnectFxOutPin(IFxPin* pFxPin, Bool ShouldStopCallBack /*= TRUE*/, Bool ShouldProtect/* = TRUE*/)
{
...
Int32 CFxEngineCore::DisplayFxPropertyPage(FX_HANDLE hFx, Pvoid pvWndParent)
{
	...
	if(IsFxLoaded(hFx))
	{
		...
		TOGGLE_LOCK(*pFx, lock);
		hr = pFx->pFxBase->DisplayFxPropertyPage(pvWndParent);
		TOGGLE_UNLOCK(*pFx, lock);
		if( FEF_FAILED(hr) )
		{
			SetFxState((FX_PTR)pFx, FX_ERROR_STATE);
...
Int32 CFxEngineCore::GetFxFrame(FX_HANDLE hFx, const Char** ppbFxFrame)
{
	...
	if(IsFxLoaded(hFx))
	{
		...
		TOGGLE_LOCK(*pFx, lock);
		hr = pFx->pFxBase->GetFxFrame(ppbFxFrame);
		TOGGLE_UNLOCK(*pFx, lock);
		if( FEF_FAILED(hr) )
		{
			SetFxState((FX_PTR)pFx, FX_ERROR_STATE);
...
Int32 CFxEngineCore::GetFxUserInterface(FX_HANDLE hFx, Pvoid* ppvUserInterface)
{
	...
	if(IsFxLoaded(hFx))
	{
		...
		TOGGLE_LOCK(*pFx, lock);
		 hr = pFx->pFxBase->GetFxUserInterface(ppvUserInterface);
		TOGGLE_UNLOCK(*pFx, lock);
		if( FEF_FAILED(hr) )
		{
			SetFxState((FX_PTR)pFx, FX_ERROR_STATE);
...
Int32 CFxEngineCore::GetFxSubFxEngine(FX_HANDLE hFx, FX_HANDLE* phFxEngine)
{
	...
	if(IsFxLoaded(hFx))
	{
		FX_COMPONENT* pFx = static_cast<FX_COMPONENT*>(hFx);
		TOGGLE_LOCK(*pFx, lock);
		hr = pFx->pFxBase->GetFxSubFxEngine(phFxEngine);
		TOGGLE_UNLOCK(*pFx, lock);
		if( FEF_FAILED(hr) )
		{
			SetFxState((FX_PTR)pFx, FX_ERROR_STATE);
...
Void CFxEngineCore::ClearFxVector(AutoLock &lock)
{
