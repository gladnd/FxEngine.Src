...
FXENGINE_EXP Int32 FXENGINE_API FEF_GetFxRunningState(FX_HANDLE hFxEngine, FX_HANDLE hFx, FX_RUNNING_STATE* pFxRunningState)
{
	CFxEngineCore* pFxEngineCore = NULL;
	pFxEngineCore = FindFxEngineCore(hFxEngine);
	if(pFxEngineCore == NULL)
	{
		g_pLogTrace->FxTrace( "FxEngineAPI::FEF_GetFxRunningState FEF_FAILED !! (Invalid Parameter: hFxEngine)");
		return FX_INVALPARAM;
	}

	return pFxEngineCore->GetFxRunningState(hFx, pFxRunningState);
}
...
FXENGINE_EXP Int32 FXENGINE_API FEF_AttachFxObserver(FX_HANDLE hFxEngine,
					   ...
					   CFxNoteCallback* pFxNoteCallback,
					   FX_PTR dwParam,
                       FX_HANDLE* phObserverId)
{
	...
	if(pFxEngineCore == NULL)
	{
		g_pLogTrace->FxTrace( "FxEngineAPI::FEF_AttachObserver FEF_FAILED !! (Invalid Parameter: hFxEngine)");
		return FX_INVALPARAM;
	}

	return pFxEngineCore->AttachFxObserver(pFxStateCallback, pFxNoteCallback, dwParam, phObserverId);
}

FXENGINE_EXP Int32 FXENGINE_API FEF_AttachFxObserverEx(FX_HANDLE hFxEngine,
						 FXSTATECALLBACK* pFxStateCallback,
						 FXNOTECALLBACK* pFxNoteCallback,
						 FX_PTR dwParam,
                         FX_HANDLE* phObserverId)
{
	CFxEngineCore* pFxEngineCore = NULL;
	...
	if(pFxEngineCore == NULL)
	{
		g_pLogTrace->FxTrace( "FxEngineAPI::FEF_AttachObserverEx FEF_FAILED !! (Invalid Parameter: hFxEngine)");
		return FX_INVALPARAM;
	}

	return pFxEngineCore->AttachFxObserverEx(pFxStateCallback, pFxNoteCallback, dwParam, phObserverId);
}
