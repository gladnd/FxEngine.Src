...
public:
    ...
    *//*-----------------------------------------------------------------------*/
	...
	@return	FX_OK if success, otherwise an FX error code.
    *//*-----------------------------------------------------------------------*/
	virtual Int32 GetMediaTypeCount(/* [out] */Uint16* pwMediaTypeCount) PURE;
	...
	@return	FX_OK if success, otherwise an FX error code.
    *//*-----------------------------------------------------------------------*/
	virtual Int32 GetMediaType(/* [out] */PFX_MEDIA_TYPE pMediaType,
							   /* [in] */Uint16 wMediaTypeIndex) PURE;

	/*----------------------------------------------------------------------*//*!
	GetConnectedPinNumber().

	The GetConnectedPinNumber method returns the number of connected pin to it.
    @param pdwPinNumber:		Pointer to a variable that receives the value.

	@return	FX_OK if success, otherwise an FX error code.
    *//*-----------------------------------------------------------------------*/
	virtual Int32 GetConnectedPinNumber(/* [out] */Uint32 *pdwPinNumber) PURE;
    ...
    *//*-----------------------------------------------------------------------*/
	...
	The GetNextPinConnected method retrieves the next Fx pins which are
	connected to it. 
    @param ppFxPin:		Address of a variable that receives a pointer
						to an IFxPin interface. Contains null pointer if
						there is no more pin connected.

	@return	FX_OK if success, otherwise an FX error code.
    *//*-----------------------------------------------------------------------*/
	virtual Int32 GetNextPinConnected(/* [out] */IFxPin** ppFxPin) PURE;
    ...
    *//*-----------------------------------------------------------------------*/
	virtual Int32 GetDeliveryMedia(/* [out] */IFxMedia** ppIFxMedia,
								   /* [in] */Uint32 dwTimeOut = 0) PURE;
