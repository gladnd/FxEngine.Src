...
{
public:
	CFxLine(CNodeManager *pNodeManager, CNode* pBeginPoint, CNode* pEndPoint);
	CFxLine(CNodeManager *pNodeManager, const CFxLine& Line);
	CFxLine(CNodeManager *pNodeManager, CNode** ppNode, Uint8 wNbPoint);
	virtual ~CFxLine();
...
public:
  ...
  Void SavePoints(wxOutputStream& stream);
    ...
    virtual Void PrintDraw(wxDC* pDC);
