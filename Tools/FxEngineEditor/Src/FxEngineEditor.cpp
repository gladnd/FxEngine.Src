...
#ifdef WIN32
// Fx images
const wxString STARTSTATUS = wxT("..\\Img\\start.png");
const wxString PAUSESTATUS = wxT("..\\Img\\pause.png");
const wxString STOPSTATUS  = wxT("..\\Img\\stop.png");
const wxString GREENLEDSTATUS  = wxT("..\\Img\\greenled.png");
const wxString ORANGELEDSTATUS = wxT("..\\Img\\orangeled.png");
const wxString REDLEDSTATUS    = wxT("..\\Img\\redled.png");
#else
const wxString STARTSTATUS = wxT("./Img/start.png");
const wxString PAUSESTATUS = wxT("./Img/pause.png");
const wxString STOPSTATUS  = wxT("./Img/stop.png");
...
wxBitmap     g_wxImgFxStartStatus;
wxBitmap     g_wxImgFxPauseStatus;
wxBitmap     g_wxImgFxStopStatus;

wxBitmap     g_wxImgFxGreenStatus;
wxBitmap     g_wxImgFxOrangeStatus;
wxBitmap     g_wxImgFxRedStatus;
...
bool FxEngineEditor::OnInit(void)
{
...
#if wxUSE_STATUSBAR
    g_frame->CreateStatusBar(2); //! Create Always
  ...
  //wxApp::OnInit();
    ...
    //g_frame->Maximize(); // Maximize the window
...
wxMDIChildFrame *FxEngineEditor::CreateChildFrame(wxDocument *doc, wxView *view, bool isCanvas)
{
  ...
  wxDocMDIChildFrame *subframe =
      new wxDocMDIChildFrame(doc, view, GetMainFrame(), wxID_ANY, strChildFrame,
                             ...
                             wxNO_FULL_REPAINT_ON_RESIZE | wxMAXIMIZE_BOX | wxMAXIMIZE);
...
BEGIN_EVENT_TABLE(Frame, wxDocMDIParentFrame)
	...
	EVT_SASH_DRAGGED_RANGE(ID_WINDOW_LEFT, ID_WINDOW_BOTTOM, Frame::OnSashDrag)
END_EVENT_TABLE()
...
Frame::Frame(wxDocManager *manager, wxFrame *g_frame, const wxString& title,
  ...
  _pleftWindow(NULL), _pdownWindow(NULL)
{
...
//    g_printData->SetPaperId(wxPAPER_LETTER); // for Americans
...
#ifdef WIN32
    wxString stdStandardPath = StandardPaths.GetDataDir();
	std::string str = ws2s(stdStandardPath) + "\\FxEngineEditor.conf";
#else
    wxString stdStandardPath = StandardPaths.GetExecutablePath();
    std::string str = ws2s(stdStandardPath) + ".conf";
    stdStandardPath = ::wxPathOnly(s2ws(str));
#endif
    
	/*! set wxImages */
	::wxSetWorkingDirectory(stdStandardPath);
	
    g_wxImgFxStartStatus.LoadFile(STARTSTATUS, wxBITMAP_TYPE_PNG);
    g_wxImgFxPauseStatus.LoadFile(PAUSESTATUS, wxBITMAP_TYPE_PNG);
    g_wxImgFxStopStatus.LoadFile(STOPSTATUS, wxBITMAP_TYPE_PNG);

    g_wxImgFxGreenStatus.LoadFile(GREENLEDSTATUS, wxBITMAP_TYPE_PNG);
    g_wxImgFxOrangeStatus.LoadFile(ORANGELEDSTATUS, wxBITMAP_TYPE_PNG);
    g_wxImgFxRedStatus.LoadFile(REDLEDSTATUS, wxBITMAP_TYPE_PNG);
    ...
    _pleftWindow = new wxSashLayoutWindow((wxMDIParentFrame*)this, ID_WINDOW_LEFT,
                               wxDefaultPosition, wxSize(200, 30),
                               wxNO_BORDER | wxSW_3D | wxCLIP_CHILDREN);
    _pleftWindow->SetDefaultSize(wxSize(200, 1000));
    _pleftWindow->SetOrientation(wxLAYOUT_VERTICAL);
    _pleftWindow->SetAlignment(wxLAYOUT_LEFT);
    _pleftWindow->SetBackgroundColour(wxColour(255, 255, 255));
    _pleftWindow->SetSashVisible(wxSASH_RIGHT, true);

    // A window to the down of the client window
    _pdownWindow = new wxSashLayoutWindow((wxMDIParentFrame*)this, ID_WINDOW_BOTTOM,
                               wxDefaultPosition, wxSize(200, 180),
                               wxNO_BORDER | wxSW_3D | wxCLIP_CHILDREN);
    _pdownWindow->SetDefaultSize(wxSize(1000, 140));
    _pdownWindow->SetOrientation(wxLAYOUT_HORIZONTAL);
    _pdownWindow->SetAlignment(wxLAYOUT_BOTTOM);
    _pdownWindow->SetBackgroundColour(wxColour(255, 255, 255));
    _pdownWindow->SetSashVisible(wxSASH_TOP, true);
    
    /*! The Fx States */
    _pFxNotebook = new wxNotebook(_pdownWindow, ID_FX_STATE_CTRL);
    _pFxStateText = new wxTextCtrl(_pFxNotebook, wxID_ANY, wxEmptyString, wxDefaultPosition, wxDefaultSize,
        wxTE_MULTILINE|wxSUNKEN_BORDER);
    _pFxNotebook->AddPage(_pFxStateText, wxT("Fx states"),true); 
    _pFxStateText->SetValue(_T("Select Fx to show states"));    

	/*! The Fx Notes & Messages */
	_pFxNoteText = new wxTextCtrl(_pFxNotebook, wxID_ANY, wxEmptyString, wxDefaultPosition, wxDefaultSize,
        wxTE_MULTILINE|wxSUNKEN_BORDER);
    _pFxNotebook->AddPage(_pFxNoteText, wxT("Fx notes"),true); 
    _pFxNoteText->SetValue(_T("Select Fx to get notes"));    

    /*! The Fx global infos */
    _pFxInfoText = new wxTextCtrl(_pFxNotebook, wxID_ANY, wxEmptyString, wxDefaultPosition, wxDefaultSize,
        wxTE_MULTILINE|wxSUNKEN_BORDER);
    _pFxNotebook->AddPage(_pFxInfoText, wxT("Fx declarations"),true); 
    _pFxInfoText->SetValue(_T("Select Fx to show definitions"));    
    
    /*! The Fx input pins */
    _pFxInputPinText = new wxTextCtrl(_pFxNotebook, wxID_ANY, wxEmptyString, wxDefaultPosition, wxDefaultSize,
        wxTE_MULTILINE|wxSUNKEN_BORDER);
    _pFxNotebook->AddPage(_pFxInputPinText, wxT("Fx input pins"),true); 
    _pFxInputPinText->SetValue(_T("Select Fx to show input pin definitions"));    

    /*! The Fx output pins */
    _pFxOutputPinText = new wxTextCtrl(_pFxNotebook, wxID_ANY, wxEmptyString, wxDefaultPosition, wxDefaultSize,
        wxTE_MULTILINE|wxSUNKEN_BORDER);
    _pFxNotebook->AddPage(_pFxOutputPinText, wxT("Fx output pins"),true); 
    _pFxOutputPinText->SetValue(_T("Select Fx to show output pin definitions"));    

    /*! The Fx parameters */
    _pFxParametersText = new wxTextCtrl(_pFxNotebook, wxID_ANY, wxEmptyString, wxDefaultPosition, wxDefaultSize,
        wxTE_MULTILINE|wxSUNKEN_BORDER);
    _pFxNotebook->AddPage(_pFxParametersText, wxT("Fx parameters"),true); 
    _pFxParametersText->SetValue(_T("Select Fx to show parameters"));    

    _pFxNotebook->SetSelection(0);
    ...
    _ptreeWindow = new CTreeCtrl(_pleftWindow, TreeTest_Ctrl,
...
Void Frame::InitTree()
{
	_ptreeWindow->InitTreeValues(&_strDirectories);
}
...
{
#ifdef WIN32
    (Void)wxMessageBox(wxT("FxEngine Editor 1.3\n")
					   wxT(__COMPILER__) wxT("\n")
                       wxT("Author: Sylvain Machel, SMProcess (c) 2012\n")
                       wxT("http://www.smprocess.com"), wxT("About FxEngineEditor"));
#else
    (Void)wxMessageBox(wxT("FxEngine Editor 1.3 linux\n")
                       wxT("Author: Sylvain Machel, SMProcess (c) 2012\n")
                       wxT("http://www.smprocess.com"), wxT("About FxEngineEditor"));
#endif
}
...
Void Frame::OnSashDrag(wxSashEvent& event)
{
    ...
    switch (event.GetId())
    {
        case ID_WINDOW_LEFT:
        {
            _pleftWindow->SetDefaultSize(wxSize(event.GetDragRect().width, 1000));
            break;
        }
        case ID_WINDOW_BOTTOM:
        {
            _pdownWindow->SetDefaultSize(wxSize(1000, event.GetDragRect().height));
            break;
        }
    }
...
Void Frame::UpdateFxStateWnd(std::string &strFxState)
{
    _pFxStateText->AppendText(s2ws(strFxState) + wxT("\n"));
    return;
}

Void Frame::UpdateFxStateWnd(std::list<std::string> &lFxState, std::string strFxName)
{
    _pFxStateText->Clear();
    _pFxNotebook->SetPageText(0, s2ws(strFxName) + wxT(" states"));
    std::list<std::string>::iterator It = lFxState.begin();
    while(It != lFxState.end()) {
        _pFxStateText->AppendText(s2ws((*It)) + wxT("\n"));
        It++;
    }
    return;
}

Void Frame::UpdateFxNoteWnd(std::string &strFxNote)
{
    _pFxNoteText->AppendText(s2ws(strFxNote) + wxT("\n"));
    return;
}

Void Frame::UpdateFxNoteWnd(std::list<std::string> &lFxNote, std::string strFxName)
{
    _pFxNoteText->Clear();
    _pFxNotebook->SetPageText(1, s2ws(strFxName) + wxT(" notes"));
    std::list<std::string>::iterator It = lFxNote.begin();
    while(It != lFxNote.end()) {
        _pFxNoteText->AppendText(s2ws((*It)) + wxT("\n"));
        It++;
    }
    return;
}

Void Frame::UpdateFxTextWnd(Int32 sdwWnd, wxString strText, std::string strFxName)
{
    switch(sdwWnd)
    {
    case ID_FX_INPIN_CTRL:
        _pFxInputPinText->Clear();
        //_pFxNotebook->SetPageText(2, strFxName.c_str());
        _pFxInputPinText->SetValue(strText);
        break;
    case ID_FX_OUTPIN_CTRL:
        _pFxOutputPinText->Clear();
        //_pFxNotebook->SetPageText(3, strFxName.c_str());
        _pFxOutputPinText->SetValue(strText);
        break;
    case ID_FX_INFO_CTRL:
        _pFxInfoText->Clear();
        //_pFxNotebook->SetPageText(1, strFxName.c_str());
        _pFxInfoText->SetValue(strText);
        break;
    case ID_FX_PARAM_CTRL:
        _pFxParametersText->Clear();
        //_pFxNotebook->SetPageText(4, strFxName.c_str());
        _pFxParametersText->SetValue(strText);
        break;
    }
}
