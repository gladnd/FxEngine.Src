/* __________________________________________________________________________
	
	FxEngine Framework. 
	Copyright (c) 2005, 2009 Sylvain Machel, SMProcess.

	This file is part of FxEngine Editor.
    FxEngine Editor is free software: you can redistribute it and/or modify
    it under the terms of the GNU General Public License as published by
    the Free Software Foundation, either version 3 of the License, or
    (at your option) any later version.

    FxEngine Editor is distributed in the hope that it will be useful,
    but WITHOUT ANY WARRANTY; without even the implied warranty of
    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
    GNU General Public License for more details.

    You should have received a copy of the GNU General Public License
    along with FxEngine Editor. If not, see <http://www.gnu.org/licenses/>.

   ____________________________________________http://www.SMProcess.com______ */
// menu items ids
enum
{
    MDI_QUIT = wxID_EXIT,
    MDI_NEW_WINDOW = 101,
	MDI_SAVE,
	MDI_SAVE_AS,
    MDI_REFRESH,
    MDI_CHANGE_TITLE,
    MDI_CHANGE_POSITION,
    MDI_CHANGE_SIZE,
    MDI_CHILD_QUIT,
    MDI_CHILD_OPTION,
    MDI_ABOUT = wxID_ABOUT,
    TreeTest_Ctrl = 1000,
    ID_WINDOW_LEFT = 1001,
    ID_WINDOW_BOTTOM = 1002,
    ID_FX_STATE_CTRL = 1003,
    ID_FX_INPIN_CTRL = 1004,
    ID_FX_OUTPIN_CTRL = 1005,
    ID_FX_INFO_CTRL = 1006,
    ID_FX_PARAM_CTRL = 1007,
	ID_FX_NOTE_CTRL = 1008,
    MENU_FXENGINE_VERSION,
    MENU_DRAWING_LOCK,
	MENU_SHOW_TXRX,
    MENU_INSERT_FX,
    MENU_INSERT_LINK,
    MENU_FXENGINE_START,
    MENU_FXENGINE_STOP,
    MENU_FXENGINE_PAUSE,
    MENU_SAVE,
    MENU_SAVE_AS,
    MENU_EXPORT,
    MENU_QUIT,
    MENU_PRINT,
    MENU_PAGE_SETUP,
    MENU_PREVIEW,
    CHANGE_FX_NAME,
    ADD_CONNECTION,
    DEL_CONNECTION,
    FX_PROPERTIES,
    FX_PROPERTIES_PAGE,
    FX_START_ONLY,
    STOP_FX_ONLY,
    PAUSE_FX_ONLY,
    TOOL_BAR_OPEN,
    TOOL_BAR_ZOOM_IN,
    TOOL_BAR_ZOOM_OUT,
    TOOL_BAR_SAVE,
    TOOL_BAR_ADD_FX,
    TOOL_BAR_ADD_LINK,
    TOOL_BAR_START,
    TOOL_BAR_STOP,
    TOOL_BAR_PAUSE,
    INFO_SHEET,
    INPIN_SHEET,
    OUTPIN_SHEET,
    PARAMETER_SHEET,
    COMMAND_LINE

};
