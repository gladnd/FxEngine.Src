...
public:
  ...
  Void updateFxWndName(std::string &strFxName) {_pFxStateText->SetName(s2ws(strFxName));}
  Void UpdateFxStateWnd(std::string &strFxState);
  Void UpdateFxNoteWnd(std::string &strFxNote);
  Void UpdateFxNoteWnd(std::list<std::string> &lFxNote, std::string strFxName);
  Void UpdateFxStateWnd(std::list<std::string> &lFxState, std::string strFxName);
  Void UpdateFxTextWnd(Int32 sdwWnd, wxString strText, std::string strFxName);

public:
	const wxArrayString* GetConfigDir() {return &_strDirectories;} 
 
private:
    wxSashLayoutWindow* _pleftWindow;
    wxSashLayoutWindow* _pdownWindow;
    wxNotebook*         _pFxNotebook;
    wxTextCtrl* _pFxStateText;
    wxTextCtrl* _pFxInfoText;
	wxTextCtrl* _pFxNoteText;
    wxTextCtrl* _pFxInputPinText;
    wxTextCtrl* _pFxOutputPinText;
    wxTextCtrl* _pFxParametersText;
    CTreeCtrl *_ptreeWindow;
