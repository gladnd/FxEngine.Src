...
Void    FEF_CALLBACK FxStateProc(FX_STATE FxState, FX_HANDLE hFx, FX_HANDLE hObserverId, FX_PTR dwParam);
Void    FEF_CALLBACK FxNoteProc(const std::string strNote, FX_HANDLE hFx, FX_HANDLE hObserverId, FX_PTR dwParam);
...
DrawingDocument::DrawingDocument()
{
	...
	if(FEF_FAILED(hr))
	{
		(Void)wxMessageBox(wxT("FEF_CreateFxEngine failed !!"), wxT("FxEngineEditor Error"), wxICON_ERROR);
	    return;
	}

	/*! Create the FxEngine Observer */
	_hFxState = NULL;
	FEF_AttachFxObserverEx(_hFxEngine, FxStateProc, FxNoteProc, (FX_PTR)this, &_hFxState);
	if(FEF_FAILED(hr))
	{
		(Void)wxMessageBox(wxT("FEF_AttachFxObserverEx failed !!"), wxT("FxEngineEditor Error"), wxICON_ERROR);
...
DrawingDocument::~DrawingDocument()
{
	...
	{
		/*! Close observer */
		hr = FEF_DetachFxObserver(_hFxEngine, _hFxState);
		if(FEF_FAILED(hr))
...
Void DrawingDocument::CloseDocument()
{
	...
	for ( Itmap = _FxMap.begin( ); Itmap != _FxMap.end( ); Itmap++ )
	{
		...
		if(_hFxEngine)
		{
			hr = FEF_StopFx(_hFxEngine, hFx);
			if(FEF_FAILED(hr))
			{
				(Void)wxMessageBox(wxT("FEF_StopFx failed !!"), wxT("FxEngineEditor Error"), wxICON_ERROR);
				continue;
			}
...
/*! Called by OnDraw */
Bool DrawingDocument::ProcessState(Frame *pMainFrame, FX_HANDLE _hLastSelectedFx)
{
    Bool _ShouldTryConnection = FALSE;
    FxMap::iterator ItFx;

    std::vector<FX_EVENT>::iterator ItList;
    ItList = _FxReceivedStates.begin();
    while(ItList != _FxReceivedStates.end()) {
        /*! Retrieve Fx */
        ItFx = _FxMap.find(ItList->_hFx);
        if(ItFx != _FxMap.end()) {
            if(ItList->_FxState == FX_PIN_UPDATE) { /*! FX_PIN_UPDATE */
                if(ItFx->second->UpdateFxPin() == TRUE)
                    _ShouldTryConnection = TRUE; //!< Try to connect pin
            }
            else if((*ItList)._FxState == FX_FRAME_UPDATE) { /*! FX_FRAME_UPDATE */
                ItFx->second->UpdateFxFrame();
            }
            else if( (ItList->_FxState == FX_CONNECT_STATE) || (ItList->_FxState == FX_DISCONNECT_STATE) ) { //!< Set the available media type or the media type of the connexion
                    ItFx->second->SetFxInPinText();
                    ItFx->second->SetFxOutPinText();
            }
            /*! Just add the state to the FX */
			ItFx->second->AddState((*ItList)._strEvent);

            /*! Update Fx States window */
			if( (_hLastSelectedFx != NULL) && (ItFx->first/*ItFx->second->GethFxHandle()*/ == _hLastSelectedFx) ) {
                pMainFrame->UpdateFxStateWnd((*ItList)._strEvent);
                if(ItList->_FxState == FX_PIN_UPDATE) { //!< Pin number has changed
                    pMainFrame->UpdateFxTextWnd(ID_FX_INPIN_CTRL, ItFx->second->GetFxInputPinText(), ItFx->second->GetFxName().c_str());
                    pMainFrame->UpdateFxTextWnd(ID_FX_OUTPIN_CTRL, ItFx->second->GetFxOutPinText(), ItFx->second->GetFxName().c_str());
                }
                else if( (ItList->_FxState == FX_CONNECT_STATE) || (ItList->_FxState == FX_DISCONNECT_STATE) ) { //!< Set the available media type or the media type of the connexion
                    ItFx->second->SetFxInPinText();
                    ItFx->second->SetFxOutPinText();
    ...
    _FxReceivedStates.clear();

	std::vector<FX_EVENT>::iterator ItNote;
    ItNote = _FxReceivedNotes.begin();
    while(ItNote != _FxReceivedNotes.end()) {
        /*! Retrieve Fx */
        ItFx = _FxMap.find(ItNote->_hFx);
        if(ItFx != _FxMap.end()) {
			/*! Just add the state to the FX */
			ItFx->second->AddNote((*ItNote)._strEvent);

            /*! Update Fx States window */
			if( (_hLastSelectedFx != NULL) && (ItFx->first/*ItFx->second->GethFxHandle()*/ == _hLastSelectedFx) )
                pMainFrame->UpdateFxNoteWnd((*ItNote)._strEvent);
		}
		ItNote++;
	}
	 _FxReceivedNotes.clear();
    
   
    return _ShouldTryConnection;
}

Void FEF_CALLBACK FxStateProc(FX_STATE FxState, FX_HANDLE hFx, FX_HANDLE hObserverId, FX_PTR dwParam)
{
   DrawingDocument *pDoc = (DrawingDocument*)dwParam;
   
	if (!pDoc)
        return;
        
    AutoLock lockDoc(pDoc->_CSDoc);
    
    wxView *pview = pDoc->GetFirstView();
	AutoLock lock(((DrawingView*)pview)->canvas->_CSDraw);

	std::string strFxState;
	FEF_GetConstToString(FX_STATE_CONST, FxState, strFxState);
    pDoc->PushFxStates(FX_EVENT(hFx, pDoc->AddEventTime(strFxState), FxState));
#ifdef WIN32
        ((DrawingView*)pview)->canvas->Refresh();
#else
		wxPaintEvent Event;
		((DrawingView*)pview)->canvas->GetEventHandler()->AddPendingEvent( Event );
#endif
...
Void FEF_CALLBACK FxNoteProc(const std::string strNote, FX_HANDLE hFx, FX_HANDLE hObserverId, FX_PTR dwParam)
{
    ...
    pDoc->PushFxNotes(FX_EVENT(hFx, pDoc->AddEventTime(strNote)));
#ifdef WIN32
        ((DrawingView*)pview)->canvas->Refresh();
#else
		wxPaintEvent Event;
...
std::string DrawingDocument::AddEventTime(std::string strEvent)
{
	wxDateTime dt = wxDateTime::UNow();
    std::string strTime(ws2s(dt.Format(_T("%H:%M:%S:%l - "))));
    strTime += strEvent;
    return(strTime);
}
...
CNode* DrawingDocument::CreateFxLine(wxPoint& Point, CFxLine** ppFxLine)
{
	...
	CFxLine* pFxLine = new CFxLine(GetNodeMng(), pBeginPoint, pEndPoint);
    _FxLineList.push_back(pFxLine);

	_NodeManager.Link(pBeginPoint);
...
{
	if( pNode->IsEnd() )
		return _NodeManager.Link(pNode);
	/*else
...
CNode* DrawingDocument::PointInNode(wxPoint& Point, CFxLine** ppFxLine)
{
	CNode* pNode;
	/*! Is point is a FxLine */
	list<CFxLine*>::reverse_iterator rIt;
	for ( rIt = _FxLineList.rbegin( ); rIt != _FxLineList.rend( ); ++rIt )
		if((*rIt)->PointInNode(Point, &pNode) == TRUE)
		{
			*ppFxLine = (*rIt);
            _FxLineList.push_back(*rIt);
            _FxLineList.erase(--rIt.base());
            return (pNode);
		}
...
Void DrawingDocument::StopFxOnly(CFx* pFx)
{
	Int32 hr;
	hr = FEF_StopFx(_hFxEngine, pFx->GethFxHandle());
	if(FEF_FAILED(hr))
	{
    ...
    /*! Set Icon status */
    //pFx->SetFxRunningState(FX_STOP_STATE);
	return;
}

Void DrawingDocument::PauseFxOnly(CFx* pFx)
{
	Int32 hr;
	hr = FEF_PauseFx(_hFxEngine, pFx->GethFxHandle());
	if(FEF_FAILED(hr))
	{
    ...
    /*! Set Icon status */
    //pFx->SetFxRunningState(FX_PAUSE_STATE);
	return;
}

Void DrawingDocument::StartFxOnly(CFx* pFx)
{
	Int32 hr;
	hr = FEF_StartFx(_hFxEngine, pFx->GethFxHandle());
	if(FEF_FAILED(hr))
	{
    ...
    /*! Set Icon status */
    //pFx->SetFxRunningState(FX_START_STATE);
	return;
}
...
wxSTD ostream& DrawingDocument::SaveObject(wxSTD ostream& stream)
{
	...
	for ( Itmap = _FxMap.begin( ); Itmap != _FxMap.end( ); Itmap++ )
	{
		CFx* pFx = Itmap->second;
        /*! write fx parameters */
        hr = FEF_QueryFxParamInterface(_hFxEngine, Itmap->first/*pFx->GethFxHandle()*/, &pIFxParam);
        if(FEF_FAILED(hr))
            continue;
...
wxOutputStream& DrawingDocument::SaveObject(wxOutputStream& stream)
{
	...
	wsprintf(str, "FxEngineEditor1.5");
	stream.Write(str,MAX_PATH);

	ZeroMemory(str, MAX_PATH * sizeof(Char));
	wsprintf(str, __COMPILER__);
	stream.Write(str,MAX_PATH);
...
wxSTD istream& DrawingDocument::LoadObject(wxSTD istream& stream)
{
	...
	const wxArrayString* pstrConfDirectories = GetMainFrame()->GetConfigDir();
    ...
    if(strcmp("FxEngineEditor", strcFxPath) == 0)
        ...
        is14Version = TRUE;
	else if(strcmp("FxEngineEditor1.5", strcFxPath) == 0)
        is15Version = TRUE;

	if( !is15Version && !is14Version && !is13Version && !is12Version && !is11Version && !is10Version)
	{
		wxMessageBox(wxT("Invalid FxEngineEditor file !!"), wxT("FxEngineEditor Error"), wxICON_ERROR);
		return stream;
	}

    wxProgressDialog dialogProg(_T("Loading Fx ....                                                            "),
                            _T("Please Wait                                                                "),
                            100,    // range
                            NULL,   // parent
                            wxPD_AUTO_HIDE |
	...
	wxMilliSleep(50); //!< just to see progress bar !!
    dialogProg.Update(0);
	...
	for(Uint32 Idx = 0; Idx < dwFxCount; Idx++)
	{
		...
		if(is12Version || is13Version || is14Version || is15Version)
        {
        ...
        dialogProg.Update((fCount++/(float)dwFxCount)*100, s2ws(strFxPath));

        /*! get jump to next Fx */
        Uint32 dwJumpFx = 0;
        if(is14Version || is15Version)
		...
		{
			/*! Try using configuration paths */
			wxArrayString::const_iterator Itr;
			for(Itr = pstrConfDirectories->begin(); Itr != pstrConfDirectories->end(); Itr++)
			{
				wxString strFx, strFxConfPath;
				wxFileName strFileName; strFileName = s2ws(strFxPath);
				wxString strName = wxString(*Itr) + wxT("/") + strFileName.GetFullName();
				if(access((ws2s(strName)).c_str(), 0) == 0){
					IsFind = TRUE;
					strFxPath = ws2s(strName);
					break; //!< Stop on first 
				}
			}
        ...
        if(is12Version || is13Version || is14Version || is15Version)
        {
			...
			pFx = new CFx(GetNodeMng(), _hFxEngine, hFx, strFxPath, dwFxPosX, dwFxPosY, strFxName);
			pFx->InitFxPin();
        }
        else if(is11Version)
        {
			...
			pFx = new CFx(GetNodeMng(), _hFxEngine, hFx, strFxPath, dwFxPosX, dwFxPosY, NULL);
        ...
        if(is11Version || is12Version || is13Version || is14Version || is15Version)
        {
            /*! read fx parameters */
			...
			if(is13Version || is14Version || is15Version) {
				pIFxParam->GetFxParamStringCount(&wParamCount);
				for(int i = 0; i < wParamCount; i++)
				{
					Uint16 wParamNameLenght;
    ...
    /* Lines */
    dialogProg.SetTitle(_T("Connect Fx ....                                                            "));
    dialogProg.Update(0);
    fCount = 0;
	...
	{
        dialogProg.Update((fCount++/(float)dwLineCount)*100);
		...
		for(Uint32 IdxPoint = 0; IdxPoint < dwPointCount; IdxPoint++)
		{
			...
			else if(is14Version || is15Version)
			{
				ppNode[IdxPoint] = pNode;
		...
		CFxLine* pFxLine = new CFxLine(GetNodeMng(), ppNode, 6);
...
wxInputStream& DrawingDocument::LoadObject(wxInputStream& stream)
{
  wxDocument::LoadObject(stream);
	...
	if(is15Version) {
		ZeroMemory(strcFxPath, MAX_PATH * sizeof(Char));
		stream.Read(strcFxPath,MAX_PATH);
		if(strcmp(__COMPILER__, strcFxPath) != 0)
		{
			wxString str = wxT("Invalid FxEngineEditor file.\n Must be open with the editor version:\n");
			str += wxT(strcFxPath);
			wxMessageBox(wxT(str), wxT("FxEngineEditor Error"), wxICON_ERROR);
			return stream;
		}
	}
	...
	for(Uint32 Idx = 0; Idx < dwFxCount; Idx++)
	{
		...
		if (_access(strFxPath.c_str(), 0) != 0)
		{
			...
			{
				wxString strFx, strFxConfPath;
				wxFileName strFileName(strFxPath);
				wxString strName = wxString(*Itr) + wxT("\\") + strFileName.GetFullName();
				if(_access(strName.c_str(), 0) == 0){
					IsFind = TRUE;
					strFxPath = strName;
					break; //!< Stop on first 
				}
