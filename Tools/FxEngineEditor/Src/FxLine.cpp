...
CFxLine::CFxLine(CNodeManager *pNodeManager, CNode** ppNode, Uint8 wNbPoint):
CDrawObject(LINE_OBJECT)
{
	...
	_pNodeManager->Add(_pAdjustedNodes[0]);
	_pAdjustedNodes[1] = new CAdjustNode(point, _NodeVector[2], _NodeVector[3], ADJUST_X);
	_pNodeManager->Add(_pAdjustedNodes[1]);
	_pAdjustedNodes[2] = new CAdjustNode(point, _NodeVector[3], _NodeVector[4], ADJUST_X);
	_pNodeManager->Add(_pAdjustedNodes[2]);
}

CFxLine::CFxLine(CNodeManager *pNodeManager, CNode* pBeginPoint, CNode* pEndPoint):
CDrawObject(LINE_OBJECT)
{
	_pNodeManager = pNodeManager; //!< Save the node manager

	_NodeVector.push_back(pBeginPoint);
	_pNodeManager->Add(pBeginPoint);
	_NodeVector.push_back(pEndPoint);
	_pNodeManager->Add(pEndPoint);
...
CFxLine::~CFxLine()
{
	...
	for(int i = 0; i < 3; i++) {
		_pNodeManager->UnLink(_pAdjustedNodes[i]);
		_pNodeManager->Remove(_pAdjustedNodes[i]);
	}
...
Void CFxLine::PrintDraw(wxDC* pDC)
{
    _DrawOnPrinter = TRUE;
    CFxLine::Draw(pDC);
    _DrawOnPrinter = FALSE;
...
Void CFxLine::GetBeginIFxPin(IFxPin** ppIfxPin)
{
	...
	pFirstNode = _pNodeManager->GetFirstLink(pNode);
	if(pFirstNode)
	{
...
Void CFxLine::PrepareMoving(CNode* pNode)
{
	...
	else
	{
		...
		if( pNode->IsEnd() )
			_pNodeManager->UnLinkList(pNode);
	}
}
...
Void CFxLine::OffSetObject(Int32 dwX, Int32 dwY)
{
	...
	for (Iter = _NodeVector.begin(); (Iter != _NodeVector.end()) && !(_NodeVector.empty()); Iter++)
	{
		//pNode = _pNodeManager->GetFirstLink((*Iter));
...
Bool CFxLine::Update()
{
	...
	for (Iter = _NodeVector.begin(); (Iter != _NodeVector.end()) && !(_NodeVector.empty()); Iter++)
	{
		if( (*Iter)->IsEnd() )
		{
			if(_pNodeManager->Link((*Iter)) == FALSE)
				return FALSE;
		}
		else
		{
			if(_pNodeManager->Link((*Iter), false) == FALSE)
				return FALSE;
		}
	}
	return TRUE;
