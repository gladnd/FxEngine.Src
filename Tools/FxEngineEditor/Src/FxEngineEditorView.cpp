...
Void DrawingView::OnDraw(wxDC *dc)
{
    ...
    //GetMainFrame()->SetStatusText((pDoc->GetFxEngineState()).c_str(), 1);
...
Void Canvas::OnDraw(wxDC& dc)
{
  ...
  if (view)
  {
    ...
    Frame *pframe = GetMainFrame();
    _TryConnect |= pDoc->ProcessState(pframe, _hLastSelectedFx);

    //pframe->SetStatusText(s2ws((pDoc->GetFxEngineState())),1);
...
Void Canvas::OnDraw(wxDC* pdc, wxPoint& DrawBeginPoint, Float fPrintZoom)
{
  ...
  if (view)
  {
    ...
    //GetMainFrame()->SetStatusText(s2ws((pDoc->GetFxEngineState())), 1);
...
#ifdef WIN32
	std::for_each(pDoc->GetFxLine().begin(), pDoc->GetFxLine().end(),
		std::bind2nd(std::mem_fun1(&CFxLine::PrintDraw), pdc));
#else
	std::for_each(pDoc->GetFxLine().begin(), pDoc->GetFxLine().end(),
...
bool Canvas::OnDropText(wxCoord x, wxCoord y, const wxString& data)
{
	...
	if (view)
	{
		...
		CFx* pFx = NULL;
		pFx = new CFx(pDoc->GetNodeMng(), pDoc->GetFxEngineHandle(), Fx, strFx, xDest, yDest);
		if(pFx == NULL)
		{
  ...
  /*_Child->PrepareDC(dc);*/
    ...
    if(event.RightDown())
    {
	    ...
	    else*/ if((_hCtxSelectedFx = pDoc->PointInFx(point)) != NULL)
	    {
			...
			if(pFx) {
                ...
                if(_hCtxSelectedFx != _hLastSelectedFx) {
                    std::string strFxName = pFx->GetFxName().c_str();
                    GetMainFrame()->UpdateFxStateWnd(pFx->GetFxStateMsg(), strFxName);
					GetMainFrame()->UpdateFxNoteWnd(pFx->GetFxNoteMsg(), strFxName);
                    GetMainFrame()->UpdateFxTextWnd(ID_FX_INPIN_CTRL, pFx->GetFxInputPinText(), strFxName);
                    GetMainFrame()->UpdateFxTextWnd(ID_FX_OUTPIN_CTRL, pFx->GetFxOutPinText(), strFxName);
                    GetMainFrame()->UpdateFxTextWnd(ID_FX_INFO_CTRL, pFx->GetFxInfosText(), strFxName);
                    GetMainFrame()->UpdateFxTextWnd(ID_FX_PARAM_CTRL, pFx->GetFxParamText(), strFxName);
                }
    ...
    else if(event.LeftDown())
    {
	    ...
	    else //!< Fx selected
	    {
            ...
            {
                /*! Update Fx States window */
                if(_hSelectedFx != _hLastSelectedFx){
                    std::string strFxName = pFx->GetFxName().c_str();
                    GetMainFrame()->UpdateFxStateWnd(pFx->GetFxStateMsg(), strFxName);
					GetMainFrame()->UpdateFxNoteWnd(pFx->GetFxNoteMsg(), strFxName);
                    GetMainFrame()->UpdateFxTextWnd(ID_FX_INPIN_CTRL, pFx->GetFxInputPinText(), strFxName);
    ...
    else if(event.Dragging())
    {
		...
		{
			_pCurrOverNode = pDoc->GetNodeMng()->GetNode(point);
	...
	if(_hCtxSelectedFx)
	{
		...
		for ( It = _SelectedObject.begin( ); It != _SelectedObject.end( ); It++ ) {
			if((*It)->GetObjectType() == FX_OBJECT) {
                ...
                /*! Set Icon status */
                ////((CFx*)(*It))->SetFxRunningState(FX_STOP_STATE);
                //pDoc->SetFxEngineState("-> STOP: " + ((CFx*)(*It))->GetFxName() + " <--");
			}
		}
	}
    event.Skip();
	...
	if(_hCtxSelectedFx)
	{
		...
		for ( It = _SelectedObject.begin( ); It != _SelectedObject.end( ); It++ ) {
			if((*It)->GetObjectType() == FX_OBJECT) {
                ...
                /*! Set Icon status */
                //((CFx*)(*It))->SetFxRunningState(FX_PAUSE_STATE);
                ///pDoc->SetFxEngineState("-> PAUSE: " + ((CFx*)(*It))->GetFxName() + " <--");
			}
		}
	}
    event.Skip();
	...
	if(_hCtxSelectedFx)
	{
		...
		for ( It = _SelectedObject.begin( ); It != _SelectedObject.end( ); It++ ) {
			if((*It)->GetObjectType() == FX_OBJECT) {
                ...
                /*! Set Icon status */
                //((CFx*)(*It))->SetFxRunningState(FX_START_STATE);
                //pDoc->SetFxEngineState("-> START: " + ((CFx*)(*It))->GetFxName() + " <--");
			}
		}
	}
    event.Skip();
...
Void Canvas::OnInsertFx(wxCommandEvent& event)
{
    ...
    if(fd.ShowModal() == wxID_OK)
    {
		if (view)
		{
			...
			CFx* pFx = NULL;
			pFx = new CFx(pDoc->GetNodeMng(), pDoc->GetFxEngineHandle(), Fx, ws2s(fd.GetPath()));
			if(pFx == NULL)
			{
				FEF_RemoveFx(pDoc->GetFxEngineHandle(), Fx);
				wxMessageBox(wxT("Internal Fx Error!!"), wxT("FxEngineEditor Error"), wxICON_ERROR);
...
Void Canvas::OnFxEngineStart(wxCommandEvent& event)
{
	...
	if (view)
	{
        ...
        //FxMap::iterator Itmap;
        //for ( Itmap = pDoc->GetFxMap().begin( ); Itmap != pDoc->GetFxMap().end( ); Itmap++ )
		    //Itmap->second->SetFxRunningState(FX_START_STATE);			

        //pDoc->SetFxEngineState("-> START FXENGINE<-");
        //GetMainFrame()->SetStatusText(s2ws((pDoc->GetFxEngineState())), 1);
        Refresh();
	}
    //event.Skip();
...
Void Canvas::OnFxEngineStop(wxCommandEvent& event)
{
	...
	if (view)
	{
        ...
        //FxMap::iterator Itmap;
        //for ( Itmap = pDoc->GetFxMap().begin( ); Itmap != pDoc->GetFxMap().end( ); Itmap++ )
		    //Itmap->second->SetFxRunningState(FX_STOP_STATE);

        //pDoc->SetFxEngineState("-> STOP FXENGINE<-");
        //GetMainFrame()->SetStatusText(s2ws((pDoc->GetFxEngineState())), 1);
      Refresh();
	}
    event.Skip();
...
Void Canvas::OnFxEnginePause(wxCommandEvent& event)
{
	...
	if (view)
	{
        ...
        //FxMap::iterator Itmap;
        //for ( Itmap = pDoc->GetFxMap().begin( ); Itmap != pDoc->GetFxMap().end( ); Itmap++ )
		    //Itmap->second->SetFxRunningState(FX_PAUSE_STATE);

        //pDoc->SetFxEngineState("-> PAUSE FXENGINE<-");
        //GetMainFrame()->SetStatusText(s2ws((pDoc->GetFxEngineState())), 1);
        Refresh();
	}
    event.Skip();
